<script lang="ts">
	...
	let gamedreamRef: HTMLDivElement | null = null;
	...
	$effect(() => {
		...
		gamedreamRef.style.setProperty('--hue', `${gamedream.hue}`);
		gamedreamRef.style.setProperty('--sat', `${gamedream.sat}%`);
	});

	let game: Phaser.Game;
	onMount(async () => {
		assignGamedreamRef(gamedreamRef);
		game = (await import('../../games/index.svelte')).initializeGame('game-screen');
	});

	$effect(() => {
		if (chickenDinner.hasWon) {
			game.destroy(true);
		}
	});
</script>

<div class="gamedream-outer" bind:this={gamedreamRef}>
	<div class="gamedream">
